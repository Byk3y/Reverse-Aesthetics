...
const STATUS_STYLES: Record<PostStatus, string> = {
  ...
  published: "bg-[#e2f6f7] text-[var(--color-clinic-teal-dark)]",
};
  ...
  return (
    <main className="mx-auto max-w-[1180px] px-[20px] py-[36px] md:px-[32px] md:py-[48px]">
      ...
      <div className="mb-[20px] flex flex-wrap items-center justify-between gap-[14px]">
        <nav className="flex flex-wrap gap-[8px]">
          {FILTERS.map((filter) => {
            ...
            return (
              <Link
                key={filter.label}
                href={filter.value ? `/admin/posts?status=${filter.value}` : "/admin/posts"}
        ...
        <form action="/admin/posts" className="flex items-center gap-[8px]">
