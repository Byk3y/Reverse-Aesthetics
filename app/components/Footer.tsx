import Link from "next/link";

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-charcoal text-white/70">
      {/* Main Footer */}
      <div className="mx-auto max-w-7xl px-6 lg:px-12 py-20 lg:py-28">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8">
          {/* Brand Column */}
          <div className="lg:col-span-1">
            <Link
              href="/"
              className="text-xl font-light uppercase tracking-[0.08em] text-white inline-block mb-5"
              style={{ fontFamily: "var(--font-display), serif" }}
            >
              Reverse{" "}
              <span className="italic font-normal normal-case tracking-[0.02em]">
                Aesthetics
              </span>
            </Link>
            <p className="text-white/45 text-sm leading-relaxed mb-8">
              Nigeria&apos;s leading aesthetic medical clinic. Natural transformations, expertly delivered.
            </p>
            {/* Social Icons — Editorial: minimal, no borders */}
            <div className="flex gap-5">
              <a
                href="https://www.instagram.com/reverseaesthetics"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 flex items-center justify-center text-white/40 hover:text-bronze transition-colors duration-300"
                aria-label="Instagram"
              >
                <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/reverseaesthetics"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 flex items-center justify-center text-white/40 hover:text-bronze transition-colors duration-300"
                aria-label="Facebook"
              >
                <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href="https://twitter.com/reverseaesthetics"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 flex items-center justify-center text-white/40 hover:text-bronze transition-colors duration-300"
                aria-label="Twitter"
              >
                <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="text-[11px] font-light uppercase tracking-[0.25em] text-white/35 mb-7">
              Quick Links
            </h4>
            <ul className="space-y-3">
              {[
                { href: "/about", label: "About Us" },
                { href: "/clinics", label: "Our Clinics" },
                { href: "/gallery", label: "Before & After" },
                { href: "/blog", label: "Blog" },
                { href: "/booking", label: "Book Appointment" },
                { href: "/contact", label: "Contact" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/50 hover:text-bronze text-sm transition-colors duration-300"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Popular Treatments */}
          <div>
            <h4 className="text-[11px] font-light uppercase tracking-[0.25em] text-white/35 mb-7">
              Popular Treatments
            </h4>
            <ul className="space-y-3">
              {[
                { href: "/treatments", label: "All Treatments" },
                { href: "/treatments/botox-and-dermal-fillers-lagos", label: "Botox & Fillers" },
                { href: "/treatments/hair-transplant-nigeria", label: "Hair Transplant" },
                { href: "/treatments/medical-weight-loss-lagos", label: "Weight Loss" },
                { href: "/treatments/dental-aesthetics-lagos", label: "Dental Aesthetics" },
                { href: "/treatments/hifu-skin-tightening-nigeria", label: "HIFU Skin Tightening" },
              ].map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-white/50 hover:text-bronze text-sm transition-colors duration-300"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            <h4 className="text-[11px] font-light uppercase tracking-[0.25em] text-white/35 mb-7">
              Our Clinics
            </h4>
            <ul className="space-y-3">
              {[
                { href: "/clinics/aesthetics", label: "Aesthetics & Dermatology" },
                { href: "/clinics/weightloss", label: "Weight Loss" },
                { href: "/clinics/dental", label: "Dental Aesthetics" },
                { href: "/clinics/hair", label: "Hair Restoration" },
                { href: "/clinics/wellness", label: "Wellness & IV Therapy" },
              ].map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-white/50 hover:text-bronze text-sm transition-colors duration-300"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-[11px] font-light uppercase tracking-[0.25em] text-white/35 mb-7">
              Visit Us
            </h4>
            <div className="space-y-5">
              <div>
                <p className="text-bronze text-[10px] uppercase tracking-[0.25em] font-medium mb-1">Lagos</p>
                <p className="text-white/50 text-sm leading-relaxed">
                  Historia Mews, No. 5 Ayo Babatunde Crescent, Oniru, Lekki
                </p>
              </div>
              <div>
                <p className="text-bronze text-[10px] uppercase tracking-[0.25em] font-medium mb-1">Abuja</p>
                <p className="text-white/50 text-sm leading-relaxed">
                  Coming Soon
                </p>
              </div>
              <div className="pt-1">
                <a
                  href="tel:+2349159188094"
                  className="text-white/50 hover:text-bronze text-sm transition-colors duration-300 block mb-1"
                >
                  +234 915 918 8094
                </a>
                <a
                  href="mailto:reverseaestheticsng@gmail.com"
                  className="text-white/50 hover:text-bronze text-sm transition-colors duration-300 block"
                >
                  reverseaestheticsng@gmail.com
                </a>
              </div>
              <p className="text-white/30 text-xs">
                Mon – Sat: 9:00 AM – 7:00 PM
              </p>
            </div>
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-white/5">
        <div className="mx-auto max-w-7xl px-6 lg:px-12 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/30 text-xs tracking-wide">
            &copy; {currentYear} Reverse Aesthetics. All rights reserved.
          </p>
          <div className="flex gap-8">
            <Link href="/contact" className="text-white/30 hover:text-white/50 text-xs tracking-wide transition-colors">
              Privacy Policy
            </Link>
            <Link href="/contact" className="text-white/30 hover:text-white/50 text-xs tracking-wide transition-colors">
              Terms of Service
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
